...
-- Allow admin users to read all system settings
...
-- Allow admin users to update system settings
CREATE POLICY "Admin users can update system settings" ON system_settings
...
CREATE POLICY "Admin users can delete system settings" ON system_settings
  FOR DELETE USING (
    EXISTS (
      SELECT 1 FROM user_profiles 
      WHERE id = auth.uid() 

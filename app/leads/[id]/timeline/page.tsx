...
import { WhatsAppTemplateSelector } from '@/components/messaging/whatsapp-template-selector';
...
export default function LeadTimelinePage() {
  ...
  const loadWhatsAppSession = async () => {
    if (!leadId || !lead?.contact_phone) return;
    
    setLoadingSession(true);
    try {
      const response = await fetch(`/api/messaging/whatsapp/session-status?leadId=${leadId}&phoneNumber=${encodeURIComponent(lead.contact_phone)}`);
      const data = await response.json();
      
      if (data.success) {
        setWhatsappSession(data.sessionStatus);
      } else {
  ...
  // Update session when lead data changes
  useEffect(() => {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      // Handle WhatsApp sending via API
      if (messageType === 'chat' && lead?.contact_phone) {
        if (!whatsappSession?.canSendFreeForm) {
          throw new Error('24 saatlik mesajlaşma penceresi dışında. Lütfen template kullanın.');
        }
        
        // Send via API
        const response = await fetch('/api/messaging/whatsapp/send-message', {
          method: 'POST',
          headers: {
      ...
      const messageData = {
        ...
        status: twilioResult ? 'sent' : 'pending' as const,
        ...
        metadata: {
          ...
          session_active: whatsappSession?.sessionActive
        }
      };
      ...
      // Reload WhatsApp session status
  ...
  const handleTemplateSelect = async (templateSid: string, variables: Record<string, string>) => {
    if (!lead?.contact_phone) return;
    
    setSendingMessage(true);
    try {
      // Send template via API
      const response = await fetch('/api/messaging/whatsapp/send-template', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (!twilioResult.success) {
        throw new Error(twilioResult.error || 'Template gönderilemedi');
      }
      
      // Template content for display (already saved in API)
      const templateContent = Object.entries(variables).reduce((content, [key, value]) => {
        return content.replace(new RegExp(`{{${key}}}`, 'g'), value);
      }, 'WhatsApp template gönderildi');
      
      // Add to timeline instantly
      const newActivity: TimelineActivity = {
        id: twilioResult.messageId || `temp-${Date.now()}`,
        type: 'message_sent',
        title: 'WhatsApp template gönderildi',
      ...
      toast({
        title: "Başarılı",
        description: "WhatsApp template başarıyla gönderildi",
      });
      ...
      toast({
        title: "Hata",
        description: error?.message || "Template gönderilirken hata oluştu",
        variant: "destructive"
      });
    } finally {
      setSendingMessage(false);
            ...
            <CardContent className="p-3">
              <Tabs value={messageType} onValueChange={(v) => setMessageType(v as any)}>
                ...
                <div className="mt-3 space-y-3">
                  ...
                  {messageType === 'chat' && lead?.contact_phone && (
                      ...
                      {loadingSession ? (
                        ...
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                            ...
                            <Badge 
                              variant={whatsappSession.canSendFreeForm ? 'default' : 'secondary'}
                              className="text-xs h-4"
                            >
                              {whatsappSession.canSendFreeForm ? 'Aktif Sohbet' : 'Template Gerekli'}
                            </Badge>
                          </div>
                          {whatsappSession.canSendFreeForm && whatsappSession.timeRemaining && (
                            <p className="text-xs text-gray-600">
                              {formatTimeRemaining(whatsappSession.timeRemaining)}
                            </p>
                          )}
                        </div>
                      ) : (
                  ...
                  <Textarea
                    placeholder={
                      messageType === 'chat' ? 
                        (whatsappSession?.canSendFreeForm ? 'WhatsApp mesajınızı yazın...' : 'Serbest mesajlaşma için template gönderin...') :
                    ...
                    disabled={messageType === 'chat' && !whatsappSession?.canSendFreeForm}
                  />
                  
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-1">
                        {/* WhatsApp Template Selector - only show for chat when template is needed */}
                        {messageType === 'chat' && lead?.contact_phone && !whatsappSession?.canSendFreeForm && (
                      ...
                      <Button 
                        ...
                        disabled={
                          sendingMessage || 
                          !messageText.trim() || 
                          (messageType === 'chat' && !whatsappSession?.canSendFreeForm)
                        }
                        size="sm"
                      >
                        <Send className="h-3 w-3 mr-1" />
                        {messageType === 'chat' && !whatsappSession?.canSendFreeForm ? 'Template Gerekli' : 'Gönder'}
                      </Button>
                    </div>
                    
                    <div className="text-xs text-muted-foreground space-y-1">
                      <p>
                        {messageType === 'chat' ? 'WhatsApp' : messageType === 'email' ? 'E-posta' : messageType === 'note' ? 'Dahili not' : 'Görev'} olarak gönderilecek
                      </p>
                      {messageType === 'chat' && !whatsappSession?.canSendFreeForm && (
                        <p className="text-orange-600">
                          ⚠️ 24 saatlik sohbet penceresi dışında. Template kullanın.
                        </p>
                      )}
                    </div>
                  </div>
